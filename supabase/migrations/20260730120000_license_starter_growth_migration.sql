-- Migration: Starter and Growth partner license updates
...
-- Add updated constraint to partner_licenses
ALTER TABLE public.partner_licenses ADD CONSTRAINT partner_licenses_license_type_check 
  CHECK (license_type IN ('basic', 'premium', 'starter', 'growth'));

-- Alter partner_license_batches to add license_type column
ALTER TABLE public.partner_license_batches
  ADD COLUMN IF NOT EXISTS license_type TEXT DEFAULT 'starter' CHECK (license_type IN ('basic', 'premium', 'starter', 'growth'));
...
-- Redefine allocate_partner_licenses
CREATE OR REPLACE FUNCTION public.allocate_partner_licenses(_partner_id UUID, _qty_starter INT, _qty_growth INT)
RETURNS void AS $$
DECLARE
  p_code TEXT;
  i INT;
...
BEGIN
  ...
  IF lic_type = 'premium' OR lic_type = 'growth' THEN
    target_plan := 'growth';
  ELSE
    target_plan := 'starter';
  END IF;
...
BEGIN
  ...
  IF NEW.license_type = 'premium' OR NEW.license_type = 'growth' THEN
    prefix := '-GROW-';
  ELSE
    prefix := '-START-';
  END IF;
...
-- Reset and re-allocate 10 starter and 5 growth licenses to all partners
DO $$
DECLARE
  p RECORD;
BEGIN
