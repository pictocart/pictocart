-- Migration: Staff management helper functions (Update, Delete, and robust Hashing)

-- 1. Re-define create_store_staff_member with 10 bcrypt rounds and secure search path
...
-- 2. Define delete_store_staff_member to safely remove the auth user
CREATE OR REPLACE FUNCTION public.delete_store_staff_member(p_staff_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth, extensions, pg_temp
...
DECLARE
  v_user_id uuid;
BEGIN
  -- Get user_id of the staff member
  ...
  -- Delete from auth.users (which cascades to public.store_staff)
  DELETE FROM auth.users WHERE id = v_user_id;

  -- Ensure store_staff is also deleted (if ON DELETE CASCADE is bypassed)
  DELETE FROM public.store_staff WHERE id = p_staff_id;
...
-- 3. Define update_store_staff_member to update employee details and optionally reset passwords
